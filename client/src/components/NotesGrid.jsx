...
export default function NotesGrid({ notes, onView, onEdit, onDelete, onDownload }) {
  return (
      ...
      {notes.map((note) => (
        <div key={note._id} className="rounded-md bg-white p-5 shadow-sm">
          ...
          <div className="mt-5 grid grid-cols-2 gap-2">
            ...
            <button className="rounded-md border p-2 text-sm font-bold" onClick={() => onDelete(note._id)}>Delete</button>
            <button className="rounded-md border p-2 text-sm font-bold" onClick={() => onDownload(note)}>PDF</button>
          </div>
        </div>
      ))}
    </div>
